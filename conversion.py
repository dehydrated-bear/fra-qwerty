...
def getUnicode(unk_txt):
    """Ensure the input is a Python 3 Unicode string."""
    if isinstance(unk_txt, bytes):
        return unk_txt.decode('utf-8', errors='replace')
    return unk_txt
...
def kru2uni(kru_text):
    ...
    # – and — replacement if not surrounded by Krutidev consonants/matrās
    misplaced = re.compile(r'[\u2014\u2013]')
    for m in misplaced.finditer(kru_text):
        index = m.start()
        if index < len(kru_text) - 1 and kru_text[index + 1] not in krutidev_consonants + krutidev_unattached_vowel_signs:
            kru_text = kru_text[:index] + u'&' + kru_text[index + 1:]

    # Apply all k2u mappings
    for mapping in k2u:
        kru_text = kru_text.replace(mapping[0], mapping[1])

    # Some special fixes
    kru_text = kru_text.replace(u'\xb1', u'Z\u0902')
    kru_text = kru_text.replace(u'\xc6', u'\u0930\u094df')

    # f + ? -> ? + ि
    misplaced = re.search('f(.?)', kru_text)
    while misplaced:
        char = misplaced.group(1)
        kru_text = kru_text.replace('f' + char, char + u'\u093f')
        misplaced = re.search('f(.?)', kru_text)
    ...
    while misplaced:
        char = misplaced.group(1)
        kru_text = kru_text.replace('fa' + char, char + u'\u093f\u0902')
        misplaced = re.search('fa(.?)', kru_text)
    ...
    while misplaced:
        char = misplaced.group(1)
        kru_text = kru_text.replace(u'\u093f\u094d' + char, u'\u094d' + char + u'\u093f')
        misplaced = re.search(u'\u093f\u094d(.?)', kru_text)

    kru_text = kru_text.replace(u'\u094dZ', u'Z')

    # Reorder र + ् before matrās
    ...
    while misplaced:
        ...
        kru_text = kru_text.replace(char + 'Z', u'\u0930\u094d' + char)
        misplaced = re.search('(.?)Z', kru_text)

    # Clean up illegal characters before matrās
    ...
    # Merge duplicate halants
    kru_text = kru_text.replace(u'\u094d\u094d\u0930', u'\u094d\u0930')
    kru_text = kru_text.replace(u'\u094d\u0930\u094d', u'\u0930\u094d')
    kru_text = kru_text.replace(u'\u094d\u094d', u'\u094d')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', type=argparse.FileType('r', encoding='utf-8'), dest='input_file', help='Input File', default=sys.stdin)
    parser.add_argument('-o', '--output', type=argparse.FileType('w', encoding='utf-8'), dest='output_file', help='Output File', default=sys.stdout)
